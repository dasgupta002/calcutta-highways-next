export default function Tile({ index, post }) {
    ...
    return (
        <section className = "flex flex-col items-center md:flex-row gap-6 md:gap-12 border-[#fbe9b0] border-b-2 px-3 py-6 mb-12">
            ...
            {
                image && <img src = { 'https:' + image.fields.file.url } className = { index % 2 === 0 ? "h-36 w-full md:w-36 hidden md:block rounded-md" : "h-36 w-full md:w-36 hidden md:block rounded-md" } />
            }
        </section>
    )
}
